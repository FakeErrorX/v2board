...
class PaymentController extends Controller
{
    ...
    public function save(Request $request)
    {
        if (!config('v2board.app_url')) {
            abort(500, 'Please configure the site address in site settings');
        }
        $params = $request->validate([
            ...
            'handling_fee_percent.between' => 'Percentage handling fee range must be between 0.1-100'
        ]);
        if ($request->input('id')) {
            $payment = Payment::find($request->input('id'));
            if (!$payment) abort(500, 'Payment method does not exist');
    ...
    public function sort(Request $request)
    {
        $request->validate([
            ...
            'ids.required' => 'Parameter error',
            'ids.array' => 'Parameter error'
        ]);
        DB::beginTransaction();
