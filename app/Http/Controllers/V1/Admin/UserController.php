...
class UserController extends Controller
{
    ...
    public function dumpCSV(Request $request)
    {
        ...
        $data = "Email,Balance,Commission Balance,Total Traffic,Device Limit,Remaining Traffic,Package Expiration Time,Subscription Plan,Subscription URL\r\n";
        foreach($res as $user) {
            $expireDate = $user['expired_at'] === NULL ? 'Permanently Valid' : date('Y-m-d H:i:s', $user['expired_at']);
    ...
    public function generate(UserGenerate $request)
    {
        if ($request->input('email_prefix')) {
            ...
            if (User::where('email', $user['email'])->first()) {
                abort(500, 'Email already exists in the system');
            }
    ...
    private function multiGenerate(Request $request)
    {
        ...
        DB::commit();
        $data = "Account,Password,Expiration Time,UUID,Creation Time,Subscription URL\r\n";
